# 👋 Olá! Seja bem-vindo(a) ao meu perfil  

## 🚀 Sobre mim  
Sou **apaixonada por Cloud Computing e Automação de Infraestrutura**, com foco em **AWS (Amazon Web Services)**.  
Gosto de transformar processos manuais em soluções **automatizadas, seguras e escaláveis**, unindo boas práticas de **DevOps** e **Infrastructure as Code (IaC)**.  

💡 Atualmente, busco expandir meu domínio em **Terraform**, **Docker**, **GitHub Actions** e **Kubernetes**, criando ambientes cloud otimizados e reprodutíveis.  

---

## 💻 Tecnologias & Ferramentas  

### ☁️ Cloud & DevOps  
![AWS](https://img.shields.io/badge/AWS-%23FF9900.svg?style=for-the-badge&logo=amazon-aws&logoColor=white)
![Docker](https://img.shields.io/badge/docker-%230db7ed.svg?style=for-the-badge&logo=docker&logoColor=white)
![Terraform](https://img.shields.io/badge/terraform-%235835CC.svg?style=for-the-badge&logo=terraform&logoColor=white)
![GitHub Actions](https://img.shields.io/badge/GitHub%20Actions-2088FF.svg?style=for-the-badge&logo=githubactions&logoColor=white)

### 🛠️ Linguagens & Scripts  
![Bash](https://img.shields.io/badge/bash-%23121011.svg?style=for-the-badge&logo=gnu-bash&logoColor=white)
![Python](https://img.shields.io/badge/python-3776AB.svg?style=for-the-badge&logo=python&logoColor=white)
![YAML](https://img.shields.io/badge/YAML-%23ffffff.svg?style=for-the-badge&logo=yaml&logoColor=151515)

### 🔧 Ferramentas  
![Git](https://img.shields.io/badge/git-%23F05033.svg?style=for-the-badge&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/github-%23121011.svg?style=for-the-badge&logo=github&logoColor=white)
![VS Code](https://img.shields.io/badge/VS%20Code-0078d4.svg?style=for-the-badge&logo=visual-studio-code&logoColor=white)

---

## 📊 Estatísticas GitHub  

<div align="center">

![Vanessa's GitHub stats](https://github-readme-stats.vercel.app/api?username=euvanessa-prado&show_icons=true&theme=radical&hide_border=true&border_radius=10)
![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=euvanessa-prado&layout=compact&theme=radical&hide_border=true&border_radius=10)

</div>

---

## 🎯 Projetos em Destaque  

### 🏗️ [Automação AWS](https://github.com/euvanessa-prado/amazonq-cli)
Scripts e automações voltadas à criação e configuração de infraestrutura em nuvem.  

**Principais recursos:**
- 🚀 Criação automatizada de instâncias EC2  
- 🌐 Configuração de VPC e sub-redes públicas  
- 📦 Gerenciamento de buckets S3  
- 🐳 Integração com Docker  

**Tecnologias:** `AWS CLI` • `Bash` • `Docker` • `EC2` • `VPC` • `S3`

---

## 🌱 Atualmente estudando  
- 🤖 **Amazon Q Developer** — IA aplicada ao desenvolvimento  
- 🏗️ **Terraform** — infraestrutura como código  
- 🔁 **CI/CD com GitHub Actions**  
- ☸️ **Kubernetes** — orquestração de containers  
- 🪶 **Apache Airflow** — automação e orquestração de pipelines de dados  

---

## 🏅 Certificações  
![AWS Certified Cloud Practitioner](https://img.shields.io/badge/AWS%20Certified%20Cloud%20Practitioner-%23FF9900?style=for-the-badge&logo=amazonaws&logoColor=white)
![Databricks Fundamentals](https://img.shields.io/badge/Databricks%20Fundamentals-%23FF3621?style=for-the-badge&logo=databricks&logoColor=white)

🎉 **Certificação AWS Cloud Practitioner concluída com sucesso!**

---

## 📈 Objetivos para 2025  
- 🧱 Criar automações completas com **Terraform**  
- ⚙️ Desenvolver pipelines **CI/CD** robustos  
- ☁️ Aprofundar em arquitetura **AWS Multi-Region**  
- 📊 Avançar na trilha de **Engenharia de Dados e Cloud**  

---

## 📊 Gráfico de Contribuições  

<div align="center">
  
[![Vanessa's activity graph](https://github-readme-activity-graph.vercel.app/graph?username=euvanessa-prado&bg_color=0d1117&color=ff4d6d&line=ff4d6d&point=ffffff&area=true&hide_border=true)](https://github.com/ashutosh00710/github-readme-activity-graph)

</div>

---

## 🌍 Visualizações de Perfil  
<div align="center">

![Profile Views](https://komarev.com/ghpvc/?username=euvanessa-prado&color=ff69b4&style=for-the-badge)

</div>

---

## 🤝 Vamos nos conectar!  
[![GitHub](https://img.shields.io/badge/GitHub-%23121011.svg?style=for-the-badge&logo=github&logoColor=white)](https://github.com/euvanessa-prado)

---

⭐ Se curtir meus projetos, **deixe uma estrela** — seu apoio ajuda a dar visibilidade!  

---

<!--
**euvanessa-prado/euvanessa-prado** é um ✨ repositório especial ✨ porque seu `README.md` (este arquivo) aparece no seu perfil do GitHub.
-->
